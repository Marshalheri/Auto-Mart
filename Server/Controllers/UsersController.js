  ...
  try{
    ...
    //First check if the email of the new user already exist in the database...
      if ( UserHelper.isUserInDb(body) == true ) {
        let message = 'Email already exist in the database, please use a different email';
...
async loginUser(req, res){
  try {
    const { email, password } = req.body;
      ...
      var validUser = UserHelper.getUserByEmail(email)
      if ( validUser == null || validUser == undefined ) {
        let message = `There is no user with email: ${email} on the database`;
        throw new ApiErrors(message, 404);
      }
      else{
        if ( !compareSync( password, validUser.password )) {
          let message = `The password entered is not correct`;
          throw new ApiErrors(message, 404);
        }
        else {
          res.status(200).json({
...
getUserById (req, res) {
  try {
    ...
    var userById = UserHelper.getUserById(user_id);
    if (userById != null || userById != undefined) {
      ...
      let message = `The user with id: ${user_id} does not exist.`;
      throw new ApiErrors(message, 404);
    }
  } catch (err) {
    res.status(err.statusCode || 500).json({
